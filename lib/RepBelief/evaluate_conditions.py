...
def evaluate_condition(
    ...
    for row in tqdm.tqdm(condition_rows[offset:num_probs]):
        ...
        if not_found:
            ...
            choose_right = (answer_key in predicted_answer_parsed) or (
                true_answer.lower().rstrip(".") in predicted_answer_parsed.lower()
            )
            choose_wrong = (negative_answer_key in predicted_answer_parsed) or (
                wrong_answer.lower().rstrip(".") in predicted_answer_parsed.lower()
            )
        ...
        print_colored(
            f"Right: Wrong: Anomaly = ({right}: {wrong}: {anomaly}) / {idx}. Acc: {right / idx:.2f}",
            "red",
        )
    ...
    prediction = os.path.join(
        RESULTS_DIR,
        f"{init_belief}_{variable}_{condition}/Dir_{args.direction}_{args.dynamic}_{args.dvariable}/prediction_{model_name}_{temperature}_{method}_{variable}_{condition}_{offset}_{num_probs}.csv",
    )

    if not os.path.exists(
        os.path.join(
            RESULTS_DIR,
            f"{init_belief}_{variable}_{condition}/Dir_{args.direction}_{args.dynamic}_{args.dvariable}/",
        )
    ):
        os.makedirs(
            os.path.join(
