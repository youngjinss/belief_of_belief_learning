...
def probe_single_case(X_train, y_train, X_val, y_val, seed=0, verbose=False):
    clf = LogisticRegression(random_state=seed, max_iter=1000, C=10).fit(
        X_train, y_train
    )
    ...
    for i in range(N):
        for setting_name in ["true", "false"]:
            for belief_statement in ["true", "false"]:
                feats = np.load(
                    "data/results/representations/%s_%s_%s_belief/reps_0shot_%s_%s_belief_%s_%d_attention.npy"
                    % (
                        dynamic,
                        variable,
                        setting_name,
...
def probe_all(all_X, all_y, test_size=0.2, seed=0):
    ...
    all_X_train, all_X_val, y_train, y_val, ids_train, ids_test = train_test_split(
        all_X, all_y, data_ids, test_size=test_size, random_state=seed
    )
    ...
    for layer in tqdm(range(num_layers)):
        for head in range(num_heads):
            ...
            (
                train_acc_all[layer][head],
                val_acc_all[layer][head],
                roc_auc_all[layer][head],
                logloss_all[layer][head],
                clf,
...
if __name__ == "__main__":
    ...
    all_X, all_y = load_data(
        dynamic=args.dynamic, belief=args.belief, variable=args.variable
    )
    ...
    plot_heatmap(
        train_acc_all,
        "Probe Train Acc.",
        save_path="data/results/probe/%s_%s_%s_train_acc"
        % (args.dynamic, args.variable, args.belief),
    )
    plot_heatmap(
        val_acc_all,
        "Probe Val Acc.",
        save_path="data/results/probe/%s_%s_%s_val_acc"
        % (args.dynamic, args.variable, args.belief),
    )
    plot_heatmap(
        roc_auc_all,
        "ROC AUC Val",
        save_path="data/results/probe/%s_%s_%s_val_auc"
        % (args.dynamic, args.variable, args.belief),
    )

    np.save(
        "data/results/probe/%s_%s_%s_val_acc.npy"
        % (args.dynamic, args.variable, args.belief),
        val_acc_all,
    )
    np.save(
        "data/results/probe/%s_%s_%s_coef.npy"
        % (args.dynamic, args.variable, args.belief),
        coefs_all,
    )
