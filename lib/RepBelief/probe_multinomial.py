...
def probe_single_case_multinomial(
    X_train, y_train, X_val, y_test, seed=0, verbose=False
):
...
if __name__ == "__main__":
    ...
    X_all, y_o = load_data(
        dynamic=args.dynamic, belief="oracle", variable=args.variable
    )
    X_all, y_p = load_data(
        dynamic=args.dynamic, belief="protagonist", variable=args.variable
    )
    ...
    all_X_train, all_X_test, y_train, y_test, ids_train, ids_test = train_test_split(
        X_all, y_combined, data_ids, test_size=0.2, random_state=args.seed
    )
    ...
    for layer in tqdm(range(num_layers)):
        for head in range(num_heads):
            ...
            train_acc_all[layer][head], val_acc_all[layer][head], clf = (
                probe_single_case_multinomial(
                    X_train, y_train, X_test, y_test, args.seed, verbose=False
                )
            )
    ...
    plot_heatmap(
        train_acc_all,
        "Probe Train Acc.",
        save_path="data/results/probe/%s_%s_multinomial_train_acc"
        % (args.dynamic, args.variable),
    )
    plot_heatmap(
        val_acc_all,
        "Probe Val Acc.",
        save_path="data/results/probe/%s_%s_multinomial_val_acc"
        % (args.dynamic, args.variable),
    )

    np.save(
        "data/results/probe/%s_%s_multinomial_train_acc.npy"
        % (args.dynamic, args.variable),
        train_acc_all,
    )
    np.save(
        "data/results/probe/%s_%s_multinomial_val_acc.npy"
        % (args.dynamic, args.variable),
        val_acc_all,
    )
    np.save(
        "data/results/probe/%s_%s_multinomial_coef.npy" % (args.dynamic, args.variable),
        coefs_all,
    )
