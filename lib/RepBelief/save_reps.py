...
def save_condition(
    ...
    if not os.path.exists(
        os.path.join(RESULTS_DIR, f"{init_belief}_{variable}_{condition}")
    ):
    ...
    output_file = os.path.join(
        RESULTS_DIR, f"{init_belief}_{variable}_{condition}/prompts.csv"
    )
    with open(output_file, mode="w", newline="", encoding="utf-8") as file:
        ...
        for idx, row in tqdm.tqdm(enumerate(condition_rows[offset:num_probs])):
            ...
            path_h = os.path.join(
                RESULTS_DIR,
                f"{init_belief}_{variable}_{condition}/reps_{method}_{variable}_{condition}_true_{idx}_hidden.npy",
            )
            path_a = os.path.join(
                RESULTS_DIR,
                f"{init_belief}_{variable}_{condition}/reps_{method}_{variable}_{condition}_true_{idx}_attention.npy",
            )
            ...
            path_h = os.path.join(
                RESULTS_DIR,
                f"{init_belief}_{variable}_{condition}/reps_{method}_{variable}_{condition}_false_{idx}_hidden.npy",
            )
            path_a = os.path.join(
                RESULTS_DIR,
                f"{init_belief}_{variable}_{condition}/reps_{method}_{variable}_{condition}_false_{idx}_attention.npy",
            )
            np.save(path_h, state_h)
            np.save(path_a, state_a)
