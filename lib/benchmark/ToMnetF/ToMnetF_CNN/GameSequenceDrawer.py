...
def drawMap(
    WIDTH, HEIGHT, simple_map, player_position, init=True, predictions=None, actual=None
):
        ...
        for j in range(HEIGHT):
            if simple_map[i][j] != 0:
                pygame.draw.rect(
                    screen,
                    color,
                    pygame.Rect((i + 1) * STEP, (j + 1) * STEP, STEP, STEP),
                )
    ...
    pygame.draw.rect(
        screen, color, pygame.Rect(0, STEP * (HEIGHT + 1), STEP * (WIDTH + 2), STEP)
    )
    pygame.draw.rect(
        screen, color, pygame.Rect(STEP * (WIDTH + 1), 0, STEP, STEP * (WIDTH + 2))
    )
            ...
            for j in range(HEIGHT):
                if simple_map[i][j] == 10:
                    pygame.draw.circle(
                        screen,
                        color,
                        center=((i + 1) * STEP + STEP / 2, (j + 1) * STEP + STEP / 2),
                        radius=STEP / 2,
                    )
        ...
        if actual is not None:
            ...
            for cord in actual:
                pygame.draw.rect(
                    screen,
                    salmon,
                    pygame.Rect((cord[0] + 1) * STEP, (cord[1] + 1) * STEP, STEP, STEP),
                )
        pygame.draw.circle(
            screen,
            color,
            center=((posX + 1) * STEP + STEP / 2, (posY + 1) * STEP + STEP / 2),
            radius=STEP / 2,
        )
