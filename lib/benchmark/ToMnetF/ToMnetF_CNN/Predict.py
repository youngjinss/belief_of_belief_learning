...
for i in range(100):
    ...
    past_MDP_traj = trajectories_past[-1][
        :, :, :, :10
    ...
    def predict(model, past_MDP_trajectories, new_MDP_curr, new_actual_dir):
        ...
        for i in range(len(new_actual_dir)):
            with torch.no_grad():
                ...
                if player_position[0] > 13 - 1:
                    player_position[0] = 13 - 1
                if player_position[0] < 0:
                    player_position[0] = 0
                if player_position[1] > 13 - 1:
                    player_position[1] = 13 - 1
                ...
                curr = torch.concat(
                    (
                        walls_layer.unsqueeze(dim=0),
                        new_player_map.unsqueeze(dim=0),
                        goal_layer,
                    ),
                    dim=0,
                )
    ...
    actual, predictions, simple_map = predict(
        model, past_MDP_traj, new_MDP_curr, new_MDP_act
    )
    ...
    def RenderActualPredictionMap(actual, predictions, simple_map):
        ...
        if actual is not None:
            ...
            for action in predictions:
                player_pos_pred = gsd.newCords(
                    [player_pos_pred[0], player_pos_pred[1]], action
                )
