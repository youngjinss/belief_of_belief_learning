...
"""
LabMaze GridWorld environment for ToMnetF experiments
Combined from Env.py and Maze.py from the original ToMnetF implementation
...
class MazeGenerator:
    def __init__(self, width, height, random_seed, max_rooms, no_walls=False):
        ...
        self.MapEnum = [
            ...
            {"Wall": 0, "Other": 1},
            # self.GoalMap
            {"Goal A": 1, "Goal B": 2, "Goal C": 3, "Goal D": 4, "Other": 0},
        ]
        ...
        if not no_walls:
            ...
            self.maze = labmaze.FixedMazeWithRandomGoals(
                MAZE_LAYOUT, num_spawns=1, num_objects=4
            )
    ...
    def init(self):
        ...
        maze = labmaze.FixedMazeWithRandomGoals(
            entity_layer=maze_layout, num_spawns=1, num_objects=4
        )
        self.maze_layout = ""
    ...
    def getWallsLayout(self):
        return self.walls_layout

    def getGoalsLayout(self):
        return self.goals_layout
    ...
    def transformMazeToArray(self, dim_matrix):
        ...
        map = np.ones((self.world_row, self.world_col), dtype=np.int16)
...
class World:
    ...
    def reset(self):
        ...
        self.state_matrix[self.WallLayerIdx] = self.Generator.CreateStateMatrix(
            self.walls_layout, "Wall"
        )
        self.state_matrix[self.PlayerLayerIdx] = self.Generator.CreateStateMatrix(
            self.maze_layout, "Player"
        )
        self.state_matrix[self.GoalLayerIdx] = self.Generator.CreateStateMatrix(
            self.goals_layout, "Goals"
        )
        ...
        dict_map = {
            "Player": np.expand_dims(self.state_matrix[self.PlayerLayerIdx], axis=0),
    ...
    def save_initial_map(self):
        for i in range(self.height):
            ...
            for j in range(self.width):
                if (
                    self.state_matrix[self.WallLayerIdx][i, j]
                    == self.MapEnum[self.WallLayerIdx]["Wall"]
                ):
                    self.init_map[i] = self.init_map[i] + "#"
                elif (
                    self.state_matrix[self.PlayerLayerIdx][i, j]
                    == self.MapEnum[self.PlayerLayerIdx]["Player"]
                ):
                    self.init_map[i] = self.init_map[i] + "O"
                elif (
                    self.state_matrix[self.GoalLayerIdx][i, j]
                    == self.MapEnum[self.GoalLayerIdx]["Goal A"]
                ):
                    self.init_map[i] = self.init_map[i] + "A"
                elif (
                    self.state_matrix[self.GoalLayerIdx][i, j]
                    == self.MapEnum[self.GoalLayerIdx]["Goal B"]
                ):
                    self.init_map[i] = self.init_map[i] + "B"
                elif (
                    self.state_matrix[self.GoalLayerIdx][i, j]
                    == self.MapEnum[self.GoalLayerIdx]["Goal C"]
                ):
                    self.init_map[i] = self.init_map[i] + "C"
                elif (
                    self.state_matrix[self.GoalLayerIdx][i, j]
                    == self.MapEnum[self.GoalLayerIdx]["Goal D"]
                ):
    ...
    def get_sight(self, sight, observability="partial"):
        self.simple_map = np.ones(
            (self.width, self.height), dtype=np.int16
        )  # 0-wall, 1-path. Start with all path, then add walls, then add goals

        # Put walls
        for row in range(self.width):
            for col in range(self.height):
                if (
                    self.state_matrix[self.WallLayerIdx][row, col]
                    == self.MapEnum[self.WallLayerIdx]["Wall"]
                ):
                    self.simple_map[row, col] = self.objectsEnum["Wall"]

        # Put goals
        for row in range(self.width):
            for col in range(self.height):
                if (
                    self.state_matrix[self.GoalLayerIdx][row, col]
                    == self.MapEnum[self.GoalLayerIdx]["Goal A"]
                ):
                    self.simple_map[row, col] = self.objectsEnum["Goal A"]
                elif (
                    self.state_matrix[self.GoalLayerIdx][row, col]
                    == self.MapEnum[self.GoalLayerIdx]["Goal B"]
                ):
                    self.simple_map[row, col] = self.objectsEnum["Goal B"]
                elif (
                    self.state_matrix[self.GoalLayerIdx][row, col]
                    == self.MapEnum[self.GoalLayerIdx]["Goal C"]
                ):
                    self.simple_map[row, col] = self.objectsEnum["Goal C"]
                elif (
                    self.state_matrix[self.GoalLayerIdx][row, col]
                    == self.MapEnum[self.GoalLayerIdx]["Goal D"]
                ):
                    self.simple_map[row, col] = self.objectsEnum["Goal D"]
    ...
    def check_terminate(self, action):
        ...
        if self.step_count > self.max_moves:
            return [True, goal_picked]
        ...
        if action == 0 and self.players_position[0] > 0:
            ...
            return [
                True,
                goal_picked,
            ]  # This could be simplified to one big boolean expression instead of many if-else
        ...
        if (
            self.state_matrix[self.GoalLayerIdx][new_position[0], new_position[1]]
            == self.MapEnum[self.GoalLayerIdx]["Other"]
        ):
            goal_picked = 0  # Path
        elif (
            self.state_matrix[self.GoalLayerIdx][new_position[0], new_position[1]]
    ...
    def check_reward(self, action, terminate, goal_picked):
        ...
        if (terminate == True) and (goal_picked == False):
            return self.reward_punish
        ...
        if action == 0:
            ...
            print(
                "ERROR: The action is incorrect. Must be between 0 and 3, got: ", action
            )
        ...
        if (
            ...
            print(
                "ERROR: Incorrect map value! Position: ",
                new_position[0],
                ", ",
                new_position[1],
            )
    ...
    def move(self, action, terminate, goal_picked):
        ...
        self.state_matrix[self.PlayerLayerIdx][
            self.players_position[0], self.players_position[1]
        ] = self.MapEnum[self.PlayerLayerIdx][
            "Other"
        ]  # self.ObjSym["Path"]
    ...
    def delete_picked_goal(self):
        self.state_matrix[self.GoalLayerIdx][
            self.players_position[0], self.players_position[1]
        ] = self.MapEnum[self.GoalLayerIdx]["Other"]
        ...
        for row in range(self.width):
            for col in range(self.height):
                if map[row, col] != self.MapEnum[self.GoalLayerIdx]["Other"]:
                    ...
                    leftgoals[goal_name] = dict(
                        position=[row, col], value=self.goal_rewards[index]
                    )
