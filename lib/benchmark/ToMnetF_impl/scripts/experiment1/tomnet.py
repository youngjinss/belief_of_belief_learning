...
class CharNet(nn.Module):
    def __init__(
        ...
        # current_state_shape = (B, 10, 13, 13, 12) # batch, channel, height, width, time_step
        self.n = ResidualBlocks
        self.N_echar = N_echar
        self.out_channels = out_channels
        self.channels_in = channels_in
        self.B = Batch  # Batch size
        self.time_step = time_step  # sequence length = time step
        self.hidden_size_lstm = 64  # 128 # 64

        self.conv_1 = TimeDistributedConv2d(
            time_step=self.time_step
        )  # Use time step conv2d to process different lengths of sequence
