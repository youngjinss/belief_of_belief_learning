#!/bin/bash
# nohup bash shell/run_exp2.sh all > experiment2.log 2>&1 &
# Complete workflow automation for ToMnetF experiment2
# Usage: bash run_exp2.sh [data_generation|train|evaluate|visualize|all]
...
DATA_DIR="$BASE_DIR/data/experiment2"
MODELS_DIR="$BASE_DIR/models/experiment2"
RESULTS_DIR="$BASE_DIR/result/experiment2"
PLOTS_DIR="$BASE_DIR/plots/experiment2"
LOG_DIR="$BASE_DIR/log"

# Create timestamp for this run
TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
RUN_LOG_DIR="$LOG_DIR/experiment2/$TIMESTAMP"
...
print_usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  data_generation  Generate trajectory data with SR and consumption labels"
    echo "  train           Train ToMnet model for experiment 2"
    echo "  evaluate        Evaluate trained model"
    echo "  visualize       Create plots and visualizations including SR maps"
