...
class DataGenerator:
    ...
    def __init__(self, grid_size: int = 11, max_walls: int = 4, max_steps: int = 31):
        ...
        self.state_dim = grid_size * grid_size * 6  # 6 channels: walls, 4 objects, agent
    ...
    def generate_random_agent_data(self, n_agents: int, n_episodes_per_agent: int,
        ...
        for agent_id, agent in enumerate(tqdm(agents, desc="Generating agent data")):
            ...
            for query_episode_id in range(n_episodes_per_agent):
                ...
                available_episodes = [i for i in range(n_episodes_per_agent) if i != query_episode_id]
                if len(available_episodes) >= n_past:
                    past_episode_ids = np.random.choice(available_episodes, n_past, replace=False)
                else:
    ...
    def __getitem__(self, idx):
        ...
        if self.experiment_type == 'figure3':
            ...
            return {
                ...
                'current_state': torch.tensor(sample['query_state'], dtype=torch.float32),
            ...
            current_trajectory = self._process_current_trajectory(sample['current_trajectory'])
            ...
            return {
                ...
                'true_consumption': torch.tensor(sample['true_consumption'], dtype=torch.float32),
                'true_sr': torch.tensor(sample['true_sr'], dtype=torch.float32),
                'agent_id': sample['agent_id'],
                'n_past': sample['n_past']
            }
    
    def _process_past_trajectories(self, trajectories: List[TrajectoryData]) -> torch.Tensor:
...
def collate_fn(batch):
    ...
    if 'true_consumption' in batch[0]:
        result['true_consumption'] = torch.stack([item['true_consumption'] for item in batch])
