...
class BayesOptimalBaseline:
    ...
    def update_posterior(self, prior_alpha: np.ndarray, observed_actions: List[int]) -> np.ndarray:
    ...
    def evaluate_on_data(self, dataset: Dict, true_alpha: float) -> Dict[str, List[float]]:
...
class ToMnetEvaluator:
    ...
    def extract_character_embeddings(self, dataset: ToMnetDataset) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def evaluate_action_prediction(self, dataset: ToMnetDataset) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                # Compute loss
                loss = F.cross_entropy(predictions['action_logits'], batch['true_actions'])
    ...
    def evaluate_policy_prediction(self, dataset: ToMnetDataset) -> Dict[str, List[float]]:
        """Evaluate policy prediction quality using KL divergence"""
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=1, shuffle=False, collate_fn=collate_fn  # Batch size 1 for individual analysis
        )
    ...
    def evaluate_by_n_past(self, dataset: ToMnetDataset, metric: str = 'accuracy') -> Dict[int, float]:
        ...
        for n_past, indices in n_past_groups.items():
            ...
            subset_dataset = ToMnetDataset(
                {'data': subset_data, 'meta': dataset.meta},
...
def load_model_from_checkpoint(model_path: str, experiment_type: str, state_dim: int, device: str = 'cuda') -> ToMnet:
...
def evaluate_unified_results(experiment_type: str, model_paths: Union[str, Dict[str, str]], 
                           dataset_paths: Union[str, Dict[str, str]], device: str = 'cuda') -> Dict:
    ...
    for model_name, model_path in model_paths.items():
        ...
        # Load model
        model = load_model_from_checkpoint(model_path, experiment_type, state_dim, device)
        ...
        for dataset_name, dataset in datasets.items():
            ...
            if experiment_type == 'figure3':
                ...
                kl_by_n_past = evaluator.evaluate_by_n_past(test_dataset, 'kl_divergence')
...
def evaluate_model(model: ToMnet, dataset: ToMnetDataset, device: str = 'cuda') -> Dict[str, float]:
    """
...
def main():
    ...
    parser.add_argument('--output_path', default='evaluation_results.pkl', 
                       help='Path to save results')
    
    args = parser.parse_args()
    
    # Evaluate using unified function
    results = evaluate_unified_results(args.experiment, args.model_path, args.data_path, args.device)
