...
from data_generation import DataGenerator, ToMnetDataset, collate_fn, MAX_STEPS
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            if extract_embeddings and hasattr(model, "character_net"):
                ...
                char_embeddings = (
                    model.character_net(batch["past_trajectories"]).cpu().numpy()
                )
            ...
            action_likelihoods = (
                action_probs.gather(1, true_actions.unsqueeze(1))
                .squeeze(1)
                .cpu()
                .numpy()
            )
...
def compute_figure5d_data(evaluation_results: Dict, n_past_filter: int = 0) -> Dict:
    """
    ...
    if filtered_embeddings.shape[1] > 2:
        ...
        print(
            f"Reduced embeddings from {filtered_embeddings.shape[1]}D to 2D using PCA"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="N_past values to analyze",
    )

    # Output parameters
    parser.add_argument(
        "--output_dir", type=str, default="result/figure5", help="Output directory"
    )
    parser.add_argument(
        "--output_file",
        type=str,
        default="figure5_results.pkl",
    ...
    dataloader = torch.utils.data.DataLoader(
        tomnet_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn
    )
    ...
    for model_path in model_paths:
        ...
        # Print summary
        print(
            f"  Action accuracy: {np.mean(evaluation_results['action_accuracies']):.4f}"
        )
        print(
            f"  Average action likelihood: {np.mean(evaluation_results['action_likelihoods']):.4f}"
        )
        print(
            f"  Character embeddings: {len(evaluation_results['character_embeddings'])} samples"
        )
