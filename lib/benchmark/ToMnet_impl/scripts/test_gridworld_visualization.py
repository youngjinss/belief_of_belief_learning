...
def test_episode_visualization():
    ...
    print(f"- Actions taken: {[['Up', 'Down', 'Left', 'Right', 'Stay'][a] for a in actions]}")
...
def test_multiple_episodes():
    ...
    for stat in episode_stats:
        if stat['success']:
            print(f"  Episode {stat['episode']}: consumed object {stat['consumed']} in {stat['steps']} steps")
...
def test_trajectory_visualization():
    ...
    while not done and len(actions_taken) < MAX_STEPS:
        ...
        if obj_positions and random.random() < 0.7:  # 70% chance to move towards object
            ...
            target = min(obj_positions, key=lambda p: abs(p[0] - env.agent_pos[0]) + abs(p[1] - env.agent_pos[1]))
    ...
    ax.plot(trajectory[:, 1], trajectory[:, 0], 'w-', linewidth=2, alpha=0.7, label='Path')
