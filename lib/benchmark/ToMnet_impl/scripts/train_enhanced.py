...
class ToMnetTrainer:
    ...
    def __init__(
        ...
        self.optimizer = optim.AdamW(
            model.parameters(), lr=learning_rate, weight_decay=config.l2_weight
        )
    ...
    def augment_batch(self, batch):
        ...
        if "past_trajectories" in batch and batch["past_trajectories"].numel() > 0:
            noise = (
                torch.randn_like(batch["past_trajectories"]) * self.config.noise_level
            )
...
def generate_data(
    ...
    if experiment_type == "figure3":
        ...
        for alpha in alpha_values:
            ...
            print(
                f"  - Expected samples: ~{config.n_agents * config.n_episodes_per_agent}"
            )
        ...
        for low_alpha, high_alpha in extreme_pairs:
            if low_alpha in datasets and high_alpha in datasets:
                print(
                    f"Creating mixed dataset for alphas {low_alpha} and {high_alpha}..."
                )
                mixed_data = []
...
def train_unified_model(experiment_type: str, args):
    ...
    print(
        f"=== Training for {experiment_type.title()} with Enhanced Generalization ==="
    )
    print("Features:")
...
def main():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=128, help="Batch size (increased)"
    )
    parser.add_argument(
    ...
    # Training strategies
    parser.add_argument(
        "--train_individual", action="store_true", help="Train individual alpha models"
    )
    parser.add_argument(
        "--train_mixed", action="store_true", help="Train mixed/generalization models"
    )
