...
class BayesianIRLMixin:
    ...
    def p_r_given_a_impl(
        ...
        preference_grid_r = NotationUtils.create_preference_grid_r(
            n_preference_points
        )  # 선호도 grid r_apple \in [0, 10] 생성
        posterior_p_r_given_a = np.zeros_like(
            preference_grid_r
        )  # 사후 확률 분포 초기화
        ...
        for i, r_apple in enumerate(preference_grid_r):  # 각 선호도 값에 대해
            r_orange = (
                DEFAULT_MAX_VALUE - r_apple
            )  # 상대 아이템의 선호도 계산 (r_orange = 10 - r_apple로 설정)
            ...
            posterior_p_r_given_a[i] = (
                likelihood_P_b_a1 * prior_p_r[i]
            )  # 우도 × 사전분포로 비정규화 사후분포 계산
...
class PriceOptimizationMixin:
    ...
    def optimize_vector_m_star(
        ...
        """
        best_vector_m = np.array(
            [DEFAULT_MAX_VALUE / 2, DEFAULT_MAX_VALUE / 2]
        )  # 초기 가격 설정 (m_apple = m_orange = 5)
        ...
        for m_apple in np.linspace(0, DEFAULT_MAX_VALUE, n_price_points):
            ...
            expected_U_s = (
                self._calculate_E_U_s_t3(  # 각 가격에 대해 기대 수익 E[U^{t=3}_s] 계산
    ...
    def _calculate_E_U_s_t3(
        ...
        for i, r_apple in enumerate(preference_grid_r):
            ...
            U_s_t3 = np.sum(
                P_b_t3 * vector_m
            )  # \sum_i P^b(a_3=i | **m**, **r**) * m(i)
            expected_U_s += (
                posterior_p_r_given_a[i] * U_s_t3
            )  # \sum_r p^s(r|a_1) * \sum_i P^b(a_3=i | **m**, **r**) * m(i)
    ...
    def compute_Q_b_t1_kl(
        ...
        for action_a1 in range(DEFAULT_N_ITEMS):  # 각 1단계 행동 a_1에 대해:
            ...
            P_b_t3 = NotationUtils.compute_softmax_policy_P(
                q_values_stage3, beta
            )  # P^b(a_3=i | **m***)
            expected_U_b_t3 = np.sum(
                P_b_t3 * q_values_stage3
            )  # E[U^{t=3}_b] = \sum_i P^b(a_3=i | **m***) * (r(i) - m***(i))
...
class LevelLBuyer(BaseAgent, StrategicPlanningMixin):
    ...
    def __init__(
        ...
        self.seller_model = (
            seller_model  # Level-(l-1) seller model -> optimal deterministic 가정
        )
...
class LevelLSeller(BaseAgent, BayesianIRLMixin, PriceOptimizationMixin):
    ...
    def __init__(
        ...
        self.level_l = level_l
        self.buyer_model = (
            buyer_model  # Level-(l-1) buyer model -> optimal deterministic 가정
        )
    ...
    def _compute_defensive_pricing(
        ...
        for item in range(DEFAULT_N_ITEMS):  # 각 아이템별로 독립적인 가격 최적화
            ...
            for price in np.linspace(
                0, DEFAULT_MAX_VALUE, n_price_points
            ):  # 각 아이템의 가격별로
                expected_U_s = 0
                ...
                if (
                    expected_U_s > best_expected_U_s
                ):  # 가장 높은 기대 수익을 가지는 가격 선택
                    best_expected_U_s = expected_U_s
                    best_price = price
...
if __name__ == "__main__":
    ...
    agents = create_agent_hierarchy(max_level=MAX_LEVEL, beta=DEFAULT_BETA)
