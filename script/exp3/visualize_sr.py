...
def visualize_sr_on_maze(maze, sr_data_dict, timestep=0, title_prefix=""):
    ...
    fig.suptitle(f'{title_prefix}SR Values at Timestep {timestep}', fontsize=16, fontweight='bold')
        ...
        for i in range(height):
            for j in range(width):
                ...
                if cell == '#':  # Wall
                    ...
                    ax.add_patch(circle)
                    ax.text(j, i, 'A', ha='center', va='center', fontweight='bold', color='white')
                elif cell in 'ABCD':  # Goals
                    circle = patches.Circle((j, i), 0.25, color='green', alpha=0.8)
                    ax.add_patch(circle)
                    ax.text(j, i, cell, ha='center', va='center', fontweight='bold', color='white')
                elif cell in 'abcd':  # Keys
                    diamond = patches.RegularPolygon((j, i), 4, radius=0.2, 
                                                   orientation=np.pi/4, color='gold', alpha=0.8)
                    ax.add_patch(diamond)
                    ax.text(j, i, cell.upper(), ha='center', va='center', fontweight='bold', color='black', fontsize=8)
                elif cell in 'ABCD' and cell.isupper():  # Doors (uppercase)
                    rect = patches.Rectangle((j-0.3, i-0.3), 0.6, 0.6, 
                                           color='brown', alpha=0.8)
                    ax.add_patch(rect)
                    ax.text(j, i, cell, ha='center', va='center', fontweight='bold', color='white', fontsize=8)
                
                # Add SR value text if significant
                if sr_values[i, j] > 0.01:
                    ax.text(j, i-0.4, f'{sr_values[i, j]:.3f}', 
...
def analyze_test_file(filename):
    ...
    timesteps_to_show = [0, 5, 10, 15, 20] if len(sr_timesteps) > 7 else list(sr_timesteps.keys())[:4]
    
    for t in timesteps_to_show:
        if t in sr_timesteps:
            fig = visualize_sr_on_maze(maze, sr_timesteps[t], timestep=t, 
                                     title_prefix=f"{filename.split('/')[-1]} - ")
            plt.savefig(f'sr_visualization_timestep_{t}.png', dpi=150, bbox_inches='tight')
        ...
        if t in sr_timesteps:
            ...
            for gamma_key in ['SR_gamma_0.5', 'SR_gamma_0.9', 'SR_gamma_0.99']:
                if gamma_key in sr_timesteps[t]:
                    ...
                    if values:
                        print(f"  {gamma_key}: {len(values)} positions, "
                              f"sum={sum(values):.3f}, max={max(values):.3f}, "
                              f"min={min(values):.3f}")

if __name__ == "__main__":
    # Analyze the test file we looked at
    test_file = "/Users/youngjins/Desktop/codes/25_belief/belief_trading/data/exp3/test98.txt"
