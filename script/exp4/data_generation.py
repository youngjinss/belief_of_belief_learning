...
class DataGenerator:
    def __init__(self, time_step=500, w=9, h=9, d=9):
        """
        ...
        # Action spaces
        self.ACHIEVER_ACTION_SPACE = 7  # up, right, down, left, stay, pickup, toggle
    ...
    def parse_trajectory_file(self, filename: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if current_section and line.startswith("SR_gamma_"):
                ...
                if current_section == "achiever" and 'sr_data_per_timestep' in achiever_data:
                    # Find the current timestep
                    timesteps = list(achiever_data['sr_data_per_timestep'].keys())
                    if timesteps:
                        current_timestep = max(timesteps)
                        achiever_data['sr_data_per_timestep'][current_timestep][str(gamma)] = sparse_data
                elif current_section == "blocker" and 'sr_data_per_timestep' in blocker_data:
                    timesteps = list(blocker_data['sr_data_per_timestep'].keys())
                    if timesteps:
                        current_timestep = max(timesteps)
                        blocker_data['sr_data_per_timestep'][current_timestep][str(gamma)] = sparse_data
                continue
            
            # Parse achiever data
            if line.startswith("Goal Consumed Rank"):
                rank_str = line.split(":")[1].strip().strip("[]")
                achiever_data['goal_rank'] = [int(r.strip()) for r in rank_str.split(",")]
                continue
            elif line.startswith("Goal Rewards:"):
                rewards_str = line.split(":")[1].strip()
                achiever_data['goal_rewards'] = [float(r) for r in rewards_str.split(",")]
                continue
                ...
                achiever_data['consumption_labels'] = np.array([float(val) for val in consumption_str.split(",")], dtype=np.float32)
                continue
    ...
    def create_achiever_sample(self, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sr_data_per_timestep = parsed_data['achiever_data'].get('sr_data_per_timestep', {})
    ...
    def create_blocker_sample(self, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        consumption_labels = np.zeros(8, dtype=np.float32)  # [key_A, key_B, key_C, key_D, door_A, door_B, door_C, door_D]
        
        if inferred_goal_letter in self.LETTER_TO_IDX:
            door_idx = 4 + self.LETTER_TO_IDX[inferred_goal_letter]  # Door indices are 4-7
        ...
        # Get SR data if available
        sr_data_per_timestep = parsed_data['blocker_data'].get('sr_data_per_timestep', {})
    ...
    def _create_trajectory_tensor(self, maze: np.ndarray, trajectory_steps: List[Dict], agent_type: str, trajectory_length: int) -> np.ndarray:
        """Create trajectory tensor for specified agent"""
        
        seq_len = min(trajectory_length, self.MAX_TRAJECTORY_SIZE)
        trajectory = np.zeros((seq_len, self.MAZE_DEPTH, self.MAZE_HEIGHT, self.MAZE_WIDTH))
    ...
    def process_directory(self, data_dir: str) -> List[Dict[str, Any]]:
        ...
        print(f"Generated {len(all_samples)} samples ({len(all_samples)//2} achiever + {len(all_samples)//2} blocker)")
        return all_samples
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate processed data for exp4 ToMnet training")
    parser.add_argument("--data_dir", type=str, required=True, help="Directory containing trajectory files")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory for output files")
    parser.add_argument("--time_step", type=int, default=500, help="Maximum trajectory length")
    ...
    parser.add_argument("--maze_depth", type=int, default=9, help="Maze depth (channels)")
