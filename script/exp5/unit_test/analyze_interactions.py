...
class InteractionAnalyzer:
    ...
    def __init__(self, base_dir):
        ...
        self.data_dir = os.path.join(base_dir, "data", "MiniGrid-AchieverBlocker-9x9-v1")
    ...
    def parse_trajectory_file(self, file_path):
        ...
        # Determine winner based on last interactions
    ...
    def _determine_winner(self, last_achiever_interaction, last_blocker_interaction):
        """
        Determine the winner based on last interactions.
        ...
        Returns:
            str: 'achiever', 'blocker', or 'need_to_examine'
        """
    ...
    def print_summary(self, all_results):
        ...
        for dir_name, results in all_results.items():
            ...
            if results['trajectory_lengths']:
                avg_length = sum(results['trajectory_lengths']) / len(results['trajectory_lengths'])
                print(f"Average trajectory length: {avg_length:.1f}")
                print(f"Min/Max trajectory length: {min(results['trajectory_lengths'])}/{max(results['trajectory_lengths'])}")
            ...
            if results['need_to_examine_files']:
                ...
                for file_info in results['need_to_examine_files']:
                    print(f"  {file_info['filename']} (length: {file_info['trajectory_length']}, "
    ...
    def save_detailed_report(self, all_results, output_file):
        ...
        with open(output_file, 'w') as f:
            ...
            for dir_name, results in all_results.items():
                ...
                # Summary stats
                f.write(f"Files: {results['parsed_files']}/{results['total_files']} parsed successfully\n")
                ...
                f.write("\nAchiever interactions:\n")
                for interaction, count in results['achiever_interaction_counts'].most_common():
                    f.write(f"  {interaction}: {count}\n")
                
                f.write("\nBlocker interactions:\n")
                for interaction, count in results['blocker_interaction_counts'].most_common():
                    f.write(f"  {interaction}: {count}\n")
