...
def load_training_data_all_combinations(config, data_dir_base=None):
    ...
    for combo_achiever, combo_blocker in all_combinations:
        ...
        if data is not None:
            print(f"Loading existing processed training data for {combo_achiever}_{combo_blocker}...")
    ...
    if missing_combinations:
        print(f"Processed training data not found for combinations: {missing_combinations}")
...
def load_test_data_all_combinations(config, test_data_dir_base=None):
    ...
    for combo_achiever, combo_blocker in all_combinations:
        ...
        if data is not None:
            print(f"Loading existing processed test data for {combo_achiever}_{combo_blocker}...")
            existing_data[(combo_achiever, combo_blocker)] = data
...
def get_data_for_combination(all_data, achiever_type, blocker_type, data_type="training"):
    """
    ...
    if (achiever_type, blocker_type) in all_data:
        ...
        raise ValueError(f"No {data_type} data found for combination {achiever_type}_{blocker_type}. Please generate {data_type} data first.")
...
def validate_data_shape(data, data_type="data"):
    ...
    required_keys = ["trajectories", "actions", "goals", "goal_ranks", "agents", "types", "consumption_labels", "sr_labels"]
    ...
    if total_samples == 0:
        raise ValueError(f"No {data_type} found. Please generate {data_type} first using appropriate flags.")
