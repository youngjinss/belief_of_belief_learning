...
class RuleBasedAgent:
    ...
    def reset(self):
        ...
        # Reset distance tracking - initialize if not exists (bug fix for original code)
        if not hasattr(self, 'distance_history'):
            self.distance_history = []
        if not hasattr(self, 'reduction_rates'):
            self.reduction_rates = []
        if not hasattr(self, 'consecutive_reductions'):
            self.consecutive_reductions = []
        if not hasattr(self, 'timestep_counter'):
            self.timestep_counter = 0
        if not hasattr(self, 'predicted_target_color'):
