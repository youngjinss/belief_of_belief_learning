...
class Config:
    def __init__(self):
        ...
        self.env_version = 2  # Options: 1 (original), 2 (with partial observation support)
        self.width = 9
        self.height = 9
        self.env_size = (
        ...
        self.partial_view_sizes = {  # View size for partial observation mode
            "3x3": 2,
            "5x5": 3,
            "9x9": 5,
            "11x11": 7,
        ...
        self.model_dir = "results/exp8"
        self.test_data_dir = (
            None  # Will be set dynamically using get_data_path(is_test=True)
        )
        self.result_dir = "results/exp8"
        self.plot_dir = "results/exp8/plots"
        self.log_dir = "log/exp8"
        ...
        # Update env_name based on version
        if self.env_version == 2:
            self.env_name = "MiniGrid-AchieverBlocker-{size}-v2"
        else:
            self.env_name = "MiniGrid-AchieverBlocker-{size}-v1"
        ...
        self.data_config = {
            ...
            "max_n_past": 10,  # Maximum past episodes for exp8
            "n_past_min": 0,  # Minimum past episodes for exp8
            "n_past_max": 10,  # Maximum past episodes for sampling in exp8
    ...
    def get_partial_view_size(self):
        """Get partial view size based on environment size"""
        return self.partial_view_sizes.get(self.env_size, 7)
