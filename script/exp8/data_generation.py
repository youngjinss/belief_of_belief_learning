...
class DataGenerator:
    ...
    def _create_trajectory_tensor(
        ...
        if observability == "partial":
            self_states = self._apply_visibility_masking(
                self_states, trajectory_steps, agent_type, partial_view_size, steps_to_process
            )

        return self_states
    
    def _apply_visibility_masking(
        self, 
        states: np.ndarray, 
        trajectory_steps: List[Dict], 
        agent_type: str, 
        ...
        """
        Apply partial observation masking to trajectory states.
        Masks regions outside agent's field of view at each timestep.
        """
        masked_states = states.copy()
        
        for t in range(steps_to_process):
            step = trajectory_steps[t]
            
            # Get agent position for this timestep
            if agent_type == "achiever":
                agent_pos = step["achiever_pos"]
            else:  # blocker
                agent_pos = step["blocker_pos"]
            ...
            min_x = max(0, agent_x - view_radius)
            max_x = min(self.MAZE_WIDTH, agent_x + view_radius + 1)
            min_y = max(0, agent_y - view_radius) 
            max_y = min(self.MAZE_HEIGHT, agent_y + view_radius + 1)
            
            # Create visibility mask (True = visible, False = masked)
            visibility_mask = np.zeros((self.MAZE_HEIGHT, self.MAZE_WIDTH), dtype=bool)
            visibility_mask[min_y:max_y, min_x:max_x] = True
            
            # Apply masking to object channels (channels 0-7: walls, objects, colors)
            # Keep agent position channels (8-9) as they're always known
            for channel in range(8):  # Only mask object channels, not agent positions
                # Zero out regions outside the agent's view
                masked_states[t, channel, ~visibility_mask] = 0.0
