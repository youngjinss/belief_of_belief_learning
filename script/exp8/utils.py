"""
Common utility functions for exp8 scripts
...
# Make the repo root importable so `beliefrl` resolves when a script is run
# from inside script/exp8/. Unnecessary once the project is installed
# (`pip install -e .`); harmless when it already is.
import os as _os
import sys as _sys
...
# Moved into the shared core; re-exported so `from utils import ...` keeps
# working for this experiment's modules during the migration.
from beliefrl.data.loading import convert_sparse_sr_to_dense, load_data_efficient
from beliefrl.train.callbacks import EarlyStopping
from beliefrl.train.reporting import print_epoch_metrics, save_training_plots
from beliefrl.utils import set_seed
...
def generate_past_episodes_from_batch(
    ...
    (Adapted from experiment 5 for exp8 multi-agent tensor format)
...
def setup_model_and_data(
    ...
    from beliefrl.model import ToMnetLoss, create_model
