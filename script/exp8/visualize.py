...
"""
Visualization tools for AchieverBlocker ToMnet experiment (exp8)
...
class BaseEmbeddingExtractor:
    ...
    def extract_embeddings(
        self, processed_data, embedding_type="character", n_samples=None
    ):
        """
        ...
        if embedding_type not in ["character", "mental", "second_belief"]:
            raise ValueError(
                f"Invalid embedding_type: {embedding_type}. Must be one of: character, mental, second_belief"
            )

        # Validate model capabilities for specific embedding types
        if embedding_type == "mental" and not getattr(
            self.model, "use_mentalnet", False
        ):
            raise ValueError(
                f"Model does not support MentalNet. Cannot extract {embedding_type} embeddings."
            )

        if embedding_type == "second_belief" and not getattr(
            self.model, "use_second_belief", False
        ):
            raise ValueError(
                f"Model does not support SecondBeliefNet. Cannot extract {embedding_type} embeddings."
            )
    ...
    def _prepare_data_tensors(self, processed_data, n_samples=None):
        ...
        if isinstance(processed_data["self_states"], torch.Tensor):
            ...
            actions_tensor = processed_data.get(
                "self_actions"
            )  # May not exist for character embeddings
        else:
            # Chunked format (numpy arrays)
            trajectories_tensor = torch.from_numpy(
                processed_data["self_states"]
            ).float()
            ...
            actions_tensor = (
                torch.from_numpy(processed_data["self_actions"]).long()
                if "self_actions" in processed_data
                else None
            )
        ...
        if len(trajectories_tensor) != batch_size or len(goals_tensor) != batch_size:
            raise ValueError(
                f"Inconsistent tensor shapes: trajectories={len(trajectories_tensor)}, goals={len(goals_tensor)}, agents={batch_size}"
            )
    ...
    def _extract_embeddings_batch(self, data_tensors, embedding_type):
        ...
        print(
            f"Extracting {embedding_type} embeddings for {len(trajectories)} samples..."
        )
        ...
        if embedding_type in ["mental", "second_belief"] and actions is None:
            raise ValueError(
                f"Actions tensor required for {embedding_type} embeddings but not found"
            )
        ...
        with torch.no_grad():
            for start_idx in range(0, len(trajectories), self.batch_size):
                ...
                if start_idx % (self.batch_size * 10) == 0:
                    print(
                        f"Processing batch {start_idx//self.batch_size + 1}/{(len(trajectories) + self.batch_size - 1)//self.batch_size}"
                    )
                ...
                if batch_embeddings is None:
                    raise RuntimeError(
                        f"Failed to extract embeddings for batch starting at {start_idx}"
                    )
    ...
    def _extract_character_embeddings_batch(
        self,
        batch_trajectories,
        batch_goal_ranks,
        batch_agents,
    ...
    def _extract_forward_based_embeddings_batch(
        self,
        batch_trajectories,
        batch_actions,
        batch_goal_ranks,
        ...
        if (
            embedding_type == "second_belief"
            and batch_opponent_trajectories is not None
        ):
        ...
        embedding_key_map = {"mental": "mental_state", "second_belief": "second_belief"}

        embedding_key = embedding_key_map[embedding_type]
        if embedding_key not in outputs:
            raise KeyError(
                f"Expected key '{embedding_key}' not found in model outputs. Available keys: {list(outputs.keys())}"
            )
        ...
        if embedding_type == "mental":
            ...
            if embedding_tensor.dim() == 4:  # Spatial mental state
                embedding_vectors = torch.mean(
                    embedding_tensor, dim=(2, 3)
...
def prepare_common_test_data(config, n_samples=None):
    ...
    print(
        f"Loaded combined test data with {processed_data['self_states'].shape[0]} samples"
    )
...
def load_test_data_for_visualization(config):
    ...
    all_test_data = load_test_data_all_combinations(
        config, test_data_dir_base=test_data_dir_base
    )
...
def plot_mental_embeddings(
    ...
    if not model.use_mentalnet:
        print(
            "Warning: Model does not use MentalNet. Skipping mental embedding visualization."
        )
        return
...
def plot_second_belief_embeddings(
    ...
    if np.std(embeddings) < 1e-8:
        print(
            "Warning: Second belief embeddings have very low variance. Visualization may not be meaningful."
        )
        print(
            "This might indicate that the model's SecondBeliefNet is not properly trained or activated."
        )
        return False
...
if __name__ == "__main__":
    ...
    results_dir = args.result_dir or getattr(config, "result_dir", "results/exp8")
    plot_dir = args.plot_dir or getattr(config, "plot_dir", "results/exp8/plots")
    ...
    title_prefix = (
        "Single-Agent" if config.is_single_agent_mode() else "AchieverBlocker"
    )
    ...
    if embedding_types_requested:
        ...
        if model is not None:
            ...
            if (
                "character" in embedding_types_requested
                or "mental" in embedding_types_requested
            ):
            ...
            if "second_belief" in embedding_types_requested:
                test_loader_second_belief = create_second_belief_dataloader(
                    test_tensors
                )
