'''
chmod +x shell/exp2/execute_synthetic_online_multi_beta.sh
./shell/exp2/execute_synthetic_online_multi_beta.sh
'''
...
# 총 실험 수 계산
TOTAL_EXPERIMENTS=$((${#EXPLORATION_BETAS[@]} * ${#LEVEL_COMBINATIONS[@]}))
echo "총 실험 수: ${TOTAL_EXPERIMENTS}"

# PID 배열
PIDS=()

# 각 EXPLORATION_BETA와 LEVEL_COMBINATION 조합에 대해 실험 실행
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    for combo in "${LEVEL_COMBINATIONS[@]}"; do
        ...
        # 로그 파일명 (exploration_beta 정보 포함)
        log_file="${LOG_DIR}/beta${exploration_beta}_L${buyer_level}B_vs_L${seller_level}S.log"
        
        echo "시작: 탐험베타=${exploration_beta}, 구매자 L${buyer_level} vs 판매자 L${seller_level}"
        ...
        # PID 파일에 저장 (exploration_beta와 조합 정보 포함)
        echo "${pid} # beta${exploration_beta}_L${buyer_level}B_vs_L${seller_level}S" >> "${PID_FILE}"
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    for combo in "${LEVEL_COMBINATIONS[@]}"; do
        ...
        echo "  beta${exploration_beta}_L${buyer_level}B_vs_L${seller_level}S: PID ${pid}" >> "${EXPERIMENT_INFO_FILE}"
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    for combo in "${LEVEL_COMBINATIONS[@]}"; do
        buyer_level=$(echo $combo | cut -d',' -f1)
        seller_level=$(echo $combo | cut -d',' -f2)
        echo "  ${LOG_DIR}/beta${exploration_beta}_L${buyer_level}B_vs_L${seller_level}S.log"
    done
done
...
echo "  tail -f ${LOG_DIR}/beta${first_beta}_L${buyer_level}B_vs_L${seller_level}S.log"
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    for combo in "${LEVEL_COMBINATIONS[@]}"; do
        ...
        seller_level=$(echo $combo | cut -d',' -f2)
        pid="${PIDS[$experiment_idx]}"
        echo "   kill ${pid}  # beta${exploration_beta}_${buyer_level},${seller_level}"
        experiment_idx=$((experiment_idx + 1))
    done
done
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    echo "  ls ${RESULT_DIR}/*beta${exploration_beta}*"
done
...
echo "  python analyze_multi_beta_results.py --result_dir ${RESULT_DIR}"
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    for combo in "${LEVEL_COMBINATIONS[@]}"; do
        buyer_level=$(echo $combo | cut -d',' -f1)
        seller_level=$(echo $combo | cut -d',' -f2)
        echo "  ${LOG_DIR}/beta${exploration_beta}_L${buyer_level}B_vs_L${seller_level}S.log" >> "${EXPERIMENT_INFO_FILE}"
    done
done
...
실시간 로그 확인 명령어 예시:
  tail -f ${LOG_DIR}/beta${first_beta}_L${buyer_level}B_vs_L${seller_level}S.log
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    for combo in "${LEVEL_COMBINATIONS[@]}"; do
        ...
        seller_level=$(echo $combo | cut -d',' -f2)
        pid="${PIDS[$experiment_idx]}"
        echo "   kill ${pid}  # beta${exploration_beta}_${buyer_level},${seller_level}" >> "${EXPERIMENT_INFO_FILE}"
        experiment_idx=$((experiment_idx + 1))
    done
done
...
for exploration_beta in "${EXPLORATION_BETAS[@]}"; do
    echo "  ls ${RESULT_DIR}/*beta${exploration_beta}*" >> "${EXPERIMENT_INFO_FILE}"
done
...
실험 완료 후 결과 분석을 위한 Python 스크립트 실행 예시:
  python analyze_multi_beta_results.py --result_dir ${RESULT_DIR}
EOF