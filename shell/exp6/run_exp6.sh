#!/bin/bash
# nohup bash shell/exp6/run_exp6.sh all > exp6.log 2>&1 &
# Complete workflow automation for AchieverBlocker experiment 6 with ToMnet training and Type prediction
# Usage: bash run_exp6.sh [data_generation|train|evaluate|visualize|all]
...
RESULTS_DIR="$BASE_DIR/results/exp6"
LOG_DIR="$BASE_DIR/log/exp6"
...
print_usage() {
    ...
    echo ""
    echo "AchieverBlocker Experiment 6 Features:"
...
run_data_generation() {
    ...
    python script/exp6/generate.py --config_override > "$RUN_LOG_DIR/train_data_generation.log" 2>&1
...
run_test_data_generation() {
    ...
    # Run generate.py from base directory to maintain correct relative paths (same as exp4)
    python script/exp6/generate.py --config_override --random_seed "$TEST_RANDOM_SEED" --test_data > "$RUN_LOG_DIR/test_data_generation.log" 2>&1
...
run_evaluation() {
    ...
    if [ ! -f "script/exp6/evaluate.py" ]; then
    ...
    if [ -f "$RESULTS_DIR/evaluation_results_exp6.json" ]; then
        log_step "Evaluation skipped - evaluation_results_exp6.json already exists"
        return 0
    fi
...
run_visualization() {
    ...
    if [ ! -f "script/exp6/visualize.py" ]; then
    ...
    python script/exp6/visualize.py --config_override --result_dir "$RESULTS_DIR" --plot_dir "$RESULTS_DIR/plots" --plot_type "all"  > "$RUN_LOG_DIR/visualization.log" 2>&1

    # python script/exp6/visualize.py --config_override  --result_dir "results/exp6/20250715_155007/" --plot_dir "results/exp6/20250715_155007/plots" --plot_type "all"
...
# Function to check AchieverBlocker experiment 6 implementation
check_exp6_implementation() {
    log_step "Checking AchieverBlocker experiment 6 implementation..."
    
    # Check if required files exist
    if [ -f "script/exp6/config.py" ]; then
        log_step "✓ config.py exists"
    else
        log_step "✗ config.py not found"
    fi
    
    if [ -f "script/exp6/data_generation.py" ]; then
        log_step "✓ data_generation.py exists"
    else
        log_step "✗ data_generation.py not found"
    fi
    
    if [ -f "script/exp6/train.py" ]; then
        log_step "✓ train.py exists"
    else
        log_step "✗ train.py not found"
    fi
    
    if [ -f "script/exp6/tomnet.py" ]; then
    ...
    if grep -q "AchieverBlocker" "script/exp6/config.py" 2>/dev/null; then
    ...
    if grep -q "is_single_agent_mode\|KeyDoor" "script/exp6/config.py" 2>/dev/null; then
    ...
    if grep -q "fc3_type\|type.*prediction" "script/exp6/tomnet.py" 2>/dev/null; then
    ...
    if grep -q "Type:\|blocker_type" "script/exp6/data_generation.py" 2>/dev/null; then
    ...
    if grep -q "type.*loss\|ToMnetLoss" "script/exp6/train.py" 2>/dev/null; then
    ...
    if grep -q "types,\|types\]\|types_tensor" "script/exp6/evaluate.py" 2>/dev/null; then
    ...
    if grep -q "plot_type_based_embeddings_for_blockers\|blocker.*type" "script/exp6/visualize.py" 2>/dev/null; then
    ...
    log_step "AchieverBlocker experiment 6 implementation check completed"
}
