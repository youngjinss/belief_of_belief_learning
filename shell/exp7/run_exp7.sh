...
RESULTS_DIR="$BASE_DIR/results/exp7"
LOG_DIR="$BASE_DIR/log/exp7"
...
print_usage() {
    ...
    echo ""
    echo "AchieverBlocker Experiment 7 Features:"
    echo "  - Multi-agent environment (Achiever and Blocker agents)"
    echo "  - Single-agent environment (KeyDoor) when no blockers configured"
    ...
    echo "  - NEW: Integrated single/multi-agent support"
...
run_data_generation() {
    ...
    python script/exp7/generate.py --config_override > "$RUN_LOG_DIR/train_data_generation.log" 2>&1
...
run_test_data_generation() {
    ...
    # Run generate.py from base directory to maintain correct relative paths (same as exp4)
    python script/exp7/generate.py --config_override --random_seed "$TEST_RANDOM_SEED" --test_data > "$RUN_LOG_DIR/test_data_generation.log" 2>&1
...
run_training() {
    ...
    if [ -n "$DEBUG_MODE" ] && [ "$DEBUG_MODE" = "true" ]; then
        ...
        python script/exp7/train.py --config_override --save_dir "$RESULTS_DIR" --device "$DEVICE" --epochs 2 --debug > "$RUN_LOG_DIR/training.log" 2>&1
    else
        python script/exp7/train.py --config_override --save_dir "$RESULTS_DIR" > "$RUN_LOG_DIR/training.log" 2>&1
    fi
...
run_evaluation() {
    ...
    if [ ! -f "script/exp7/evaluate.py" ]; then
    ...
    if [ -f "$RESULTS_DIR/evaluation_results_exp7.json" ]; then
        log_step "Evaluation skipped - evaluation_results_exp7.json already exists"
        return 0
    fi
    ...
    if [ -n "$DEBUG_MODE" ] && [ "$DEBUG_MODE" = "true" ]; then
        ...
        python script/exp7/evaluate.py --config_override \
    ...
    # python script/exp7/evaluate.py --config_override --model_path "results/exp7/20250715_155007/best_model.pth" --test_data_dir "data/MiniGrid-AchieverBlocker-9x9-v1/value_randomly_selected_rule_based/test" --result_dir "results/exp7/20250715_155007/" --plot_type "all" --device "cuda:1"
...
run_visualization() {
    ...
    if [ ! -f "script/exp7/visualize.py" ]; then
    ...
    python script/exp7/visualize.py --config_override --result_dir "$RESULTS_DIR" --plot_dir "$RESULTS_DIR/plots" --plot_type "all"  > "$RUN_LOG_DIR/visualization.log" 2>&1

    # python script/exp7/visualize.py --config_override  --result_dir "results/exp7/20250715_155007/" --plot_dir "results/exp7/20250715_155007/plots" --plot_type "all"
...
# Function to check AchieverBlocker experiment 7 implementation
check_exp7_implementation() {
    log_step "Checking AchieverBlocker experiment 7 implementation..."
    
    # Check if required files exist
    if [ -f "script/exp7/config.py" ]; then
        log_step "✓ config.py exists"
    else
        log_step "✗ config.py not found"
    fi
    
    if [ -f "script/exp7/data_generation.py" ]; then
        log_step "✓ data_generation.py exists"
    else
        log_step "✗ data_generation.py not found"
    fi
    
    if [ -f "script/exp7/train.py" ]; then
        log_step "✓ train.py exists"
    else
        log_step "✗ train.py not found"
    fi
    
    if [ -f "script/exp7/tomnet.py" ]; then
    ...
    if grep -q "AchieverBlocker" "script/exp7/config.py" 2>/dev/null; then
    ...
    # Check if config has single-agent mode support
    if grep -q "is_single_agent_mode\|KeyDoor" "script/exp7/config.py" 2>/dev/null; then
        log_step "✓ Single-agent mode support configured in config.py"
    else
        log_step "✗ Single-agent mode support not configured in config.py"
    fi
    
    # Check if ToMnet model supports Type prediction
    if grep -q "fc3_type\|type.*prediction" "script/exp7/tomnet.py" 2>/dev/null; then
    ...
    if grep -q "Type:\|blocker_type" "script/exp7/data_generation.py" 2>/dev/null; then
    ...
    if grep -q "type.*loss\|ToMnetLoss" "script/exp7/train.py" 2>/dev/null; then
    ...
    if grep -q "types,\|types\]\|types_tensor" "script/exp7/evaluate.py" 2>/dev/null; then
    ...
    if grep -q "plot_type_based_embeddings_for_blockers\|blocker.*type" "script/exp7/visualize.py" 2>/dev/null; then
    ...
    log_step "AchieverBlocker experiment 7 implementation check completed"
}
...
get_data_paths() {
    python -c "
import sys
sys.path.append('script/exp7')
from config import Config
config = Config()
achiever_type = list(config.achiever_types.keys())[0]
# Check if single-agent mode
if config.is_single_agent_mode():
    print('TRAIN_DATA_DIR=' + config.get_data_path(achiever_type, None, is_test=False))
    print('TEST_DATA_DIR=' + config.get_data_path(achiever_type, None, is_test=True))
else:
